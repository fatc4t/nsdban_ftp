...
SET BATLOGFILE=%~dp0log\vandata_ftp_get.log
SET BATLOGTEMP=%~dp0log\vandata_ftp_get_tmp.log
SET yyyyMMdd=%date:/=%
SET hhmmss=%time: =0%
...
@rem 出力先フォルダ作成
@rem ====================================
mkdir %~dp0out\SHNINF\%yyyyMMdd%_%hhmmss%
cd    %~dp0out\SHNINF\%yyyyMMdd%_%hhmmss%
...
ftp -s:..\..\..\conf\vandata_ftp_get.conf  > %BATLOGTEMP% 2>&1
...
copy ".\shohn.102" "..\..\..\ftp_recieve\"  >> %BATLOGFILE% 2>&1
